...
THREAD_LOCAL const char program_name[] = "ffmpeg";
THREAD_LOCAL const int program_birth_year = 2000;

THREAD_LOCAL FILE *vstats_file;
...
THREAD_LOCAL Decoder     **decoders;
THREAD_LOCAL int        nb_decoders;

#if HAVE_TERMIOS_H

/* init terminal so that we can grab keys */
THREAD_LOCAL static struct termios oldtty;
THREAD_LOCAL static int restore_tty;
#endif
...
THREAD_LOCAL static volatile int received_sigterm = 0;
THREAD_LOCAL static volatile int received_nb_signals = 0;
THREAD_LOCAL static atomic_int transcode_init_done = 0;
THREAD_LOCAL static volatile int ffmpeg_exited = 0;
THREAD_LOCAL static int64_t copy_ts_first_pts = AV_NOPTS_VALUE;
...
THREAD_LOCAL const AVIOInterruptCB int_cb = { decode_interrupt_cb, NULL };
